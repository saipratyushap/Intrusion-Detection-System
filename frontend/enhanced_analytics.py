...
def show_comparison_analytics(filtered_df, df):
    ...
    if 'Timestamp' in filtered_df.columns:
        ...
        st.markdown(f"""
        <div style="background: rgba(112, 145, 56, 0.1); padding: 1rem; border-radius: 10px; margin-bottom: 1rem; color: #2D3E50;">
        ...
        if len(filtered_df) > 1 and len(previous_df) > 1:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(x=daily_current['Date'], y=daily_current['Current'], name='Current Period', marker_color='#709138'))
            fig.add_trace(go.Bar(x=daily_previous['Date'], y=daily_previous['Previous'], name='Previous Period', marker_color='#00A1C9', opacity=0.7))
            
            fig.update_layout(title="Daily Detections Comparison", xaxis_title="Date", yaxis_title="Number of Detections", barmode='group', template='plotly_white', height=400)
...
def show_trend_forecasting(filtered_df):
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(f"""<div class="glass-card" style="text-align: center; padding: 1.5rem; background: #FFFFFF; border: 1px solid #D1DBE5;"><div style="font-size: 2rem;">🎯</div><div style="font-size: 1.5rem; font-weight: bold; color: #709138;">{det_trend}</div><div style="color: #4A4A4A;">Detection Trend</div><div style="font-size: 0.9rem; margin-top: 0.5rem; color: #4A4A4A;">Slope: {model_detections.coef_[0]:.2f} per day</div></div>""", unsafe_allow_html=True)
    
    with col2:
        st.markdown(f"""<div class="glass-card" style="text-align: center; padding: 1.5rem; background: #FFFFFF; border: 1px solid #D1DBE5;"><div style="font-size: 2rem;">🚨</div><div style="font-size: 1.5rem; font-weight: bold; color: #ef4444;">{viol_trend}</div><div style="color: #4A4A4A;">Violation Trend</div><div style="font-size: 0.9rem; margin-top: 0.5rem; color: #4A4A4A;">Slope: {model_violations.coef_[0]:.2f} per day</div></div>""", unsafe_allow_html=True)
    ...
    fig = px.line(plot_df, x='Date', y='Detections', color='Type', markers=True, color_discrete_map={'Historical': '#709138', 'Forecast': '#00A1C9'})
    ...
    fig.update_layout(title="Detection Trend with Forecast", template='plotly_white', height=400, xaxis_title="Date", yaxis_title="Predicted Detections")
    st.plotly_chart(fig, use_container_width=True)
...
def show_anomaly_detection(filtered_df):
    ...
    with col1:
        ...
        if not anomalies.empty:
            ...
            for _, row in anomalies.iterrows():
                ...
                st.markdown(f"""<div style="background: rgba(239, 68, 68, 0.1); padding: 0.75rem; border-radius: 8px; margin-bottom: 0.5rem; color: #2D3E50;"><strong>{row['Date']}</strong> - {direction} activity ({int(row['Count'])} detections)<br><small style="color: #4A4A4A;">Z-Score: {z:.2f}</small></div>""", unsafe_allow_html=True)
        else:
    ...
    with col2:
        ...
        if not high_violation_days.empty:
            ...
            for _, row in high_violation_days.iterrows():
                st.markdown(f"""<div style="background: rgba(0, 161, 201, 0.1); padding: 0.75rem; border-radius: 8px; margin-bottom: 0.5rem; color: #2D3E50;"><strong>{row['Date']}</strong> - {int(row['ViolationCount'])} violations<br><small style="color: #4A4A4A;">Above average by {row['ViolationCount'] - mean_violations:.1f}</small></div>""", unsafe_allow_html=True)
        else:
    ...
    fig = go.Figure()
    fig.add_trace(go.Box(y=daily_counts['Count'], name='Daily Detections', marker_color='#709138', boxpoints='all', jitter=0.3))
    ...
    fig.update_layout(title="Daily Detection Distribution (Outliers Marked in Red)", template='plotly_white', height=350, yaxis_title="Detections per Day")
    st.plotly_chart(fig, use_container_width=True)
...
def show_interactive_charts(filtered_df):
    ...
    if chart_type == "Detection Trends Over Time":
        ...
        fig.update_traces(line_color='#709138', marker=dict(size=8))
        fig.update_layout(template='plotly_white', hovermode='x unified', dragmode='zoom')
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig = px.histogram(df_copy, x='Confidence', nbins=20, title="Confidence Score Distribution", color_discrete_sequence=['#709138'])
        fig.update_layout(template='plotly_white', bargap=0.1, xaxis_title="Confidence Score (%)", yaxis_title="Frequency")
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig.update_layout(template='plotly_white', xaxis_title="Hour of Day", yaxis_title="Confidence Score")
        st.plotly_chart(fig, use_container_width=True)
...
def show_correlation_analysis(filtered_df):
    ...
    fig.update_layout(template='plotly_white', xaxis_title="Variable", yaxis_title="Variable")
    st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig.update_layout(template='plotly_white', showlegend=False, xaxis_title="Day of Week", yaxis_title="Confidence (%)")
        st.plotly_chart(fig, use_container_width=True)
...
def show_enhanced_analytics(filtered_df, df):
    ...
    .section-header {
        ...
        border-bottom: 2px solid #709138;
    }
    </style>
    """, unsafe_allow_html=True)
