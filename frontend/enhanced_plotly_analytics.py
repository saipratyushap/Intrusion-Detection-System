...
TEXT_COLOR = '#2D3E50'  # Dark Slate for readability
BG_COLOR = '#FFFFFF'
GRID_COLOR = 'rgba(0,0,0,0.05)'

def apply_enhanced_layout(fig, title, height=450):
    """Apply consistent styling to all Plotly figures"""
    fig.update_layout(
        title=dict(
            text=f"<b>{title}</b>",
            font=dict(size=22, color='#709138', family="Inter, sans-serif"),
            x=0.01,
            y=0.96
        ),
        ...
        xaxis=dict(
            ...
            title_font=dict(color=TEXT_COLOR, size=12, weight='bold'),
            showgrid=True
        ),
        yaxis=dict(
            gridcolor=GRID_COLOR,
...
def create_detection_timeline(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Timeline')
    return fig
...
def create_detection_class_bar_chart(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Classes Distribution')
...
def create_confidence_distribution(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Confidence Score Distribution')
...
def create_detection_heatmap(detection_data, time_period='hourly'):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Heatmap (Hour vs Date)', height=500)
    fig.update_layout(xaxis_title='Hour of Day', yaxis_title='Date')
    return fig
...
def create_detection_radar_chart(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Classes - Radar View', height=500)
    return fig
...
def create_confidence_gauge(current_confidence, label="Average Confidence"):
    ...
    fig = apply_enhanced_layout(fig, label)
    return fig
...
def create_class_confidence_box_plot(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Confidence Score Distribution by Class')
    fig.update_layout(xaxis_title='Class', yaxis_title='Confidence Score')
    return fig
...
def create_detection_pie_chart(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Classes Distribution (Pie)', height=500)
    return fig
...
def create_detection_scatter_plot(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Confidence Over Time')
...
def create_hourly_trend(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Hourly Detection Trend')
    fig.update_layout(xaxis_title='Hour of Day', yaxis_title='Number of Detections')
    return fig
...
def create_daily_trend(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Daily Detection Trend')
    fig.update_layout(xaxis_title='Date', yaxis_title='Number of Detections')
    return fig
...
def create_class_timeline(detection_data):
    ...
    fig = apply_enhanced_layout(fig, 'Detection Classes Timeline (Stacked)')
    fig.update_layout(xaxis_title='Date', yaxis_title='Number of Detections')
    return fig
...
def create_comparison_dashboard(detection_data, metric1='class', metric2='confidence'):
    ...
    fig = apply_enhanced_layout(fig, 'Analytics Dashboard', height=800)
