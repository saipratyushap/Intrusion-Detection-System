document.addEventListener("DOMContentLoaded", function () {
    ...
    // Bar Chart Initialization (needs to be defined before connectWebSocket)
    ...
    var violationPieChart = new Chart(pieCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#709138', '#00A1C9', '#2D3E50'],
                borderColor: '#FFFFFF',
                borderWidth: 2
            }]
        },
    ...
    function connectWebSocket() {
        ...
        ws.onmessage = function (event) {
            try {
                ...
                if (totalDetections) totalDetections.innerHTML = data.summary.total_detections;
                if (totalViolations) totalViolations.innerHTML = data.summary.total_violations;
                if (mostFrequentClass) mostFrequentClass.innerHTML = data.summary.most_frequent_class;
                ...
                data.timestamp.forEach(function (timestamp, index) {
                    ...
                    // Add new row to the table (limit to 100 rows for performance)
                    var newRow = `<tr>
                        <td>${timestamp}</td>
                        <td>${detectedClass}</td>
                    ...
                    if (dataTable) {
                        ...
                        if (dataTable.rows.length > 100) {
                            dataTable.deleteRow(100);
                        }
                    }
                });
    ...
    function updateStatusIndicator(connected) {
        ...
        if (indicator) {
            if (connected) {
                ...
                indicator.innerHTML = '<i class="fas fa-circle" style="color: #94a3b8;"></i> Connecting...';
...
function updateChartColors(theme) {
    const textColor = theme === 'dark' ? '#FFFFFF' : '#2D3E50';
...
async function startMonitoring() {
    try {
        const response = await JWTAuth.authenticatedFetch('/api/monitoring/start', { method: 'POST' });
        if (response.ok) {
...
async function stopMonitoring() {
    try {
        const response = await JWTAuth.authenticatedFetch('/api/monitoring/stop', { method: 'POST' });
        if (response.ok) {
...
async function muteAlerts() {
    try {
        const response = await JWTAuth.authenticatedFetch('/api/alerts/mute', { method: 'POST' });
        if (response.ok) {
...
async function unmuteAlerts() {
    try {
        const response = await JWTAuth.authenticatedFetch('/api/alerts/unmute', { method: 'POST' });
        if (response.ok) {
