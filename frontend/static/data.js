...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Using sequential loading to prevent browser hanging
    loadInitialDataSequential();

    // Setup WebSocket connection with timeout protection
    ...
    // Start auto-refresh interval (30 seconds instead of 10 to reduce load)
    startAutoRefresh();

    // Update summary stats periodically (every 10 seconds instead of 5)
    setInterval(updateSummaryStatsOptimized, 10000);
});

// Sequential loading to prevent browser hanging
async function loadInitialDataSequential() {
    console.log('Loading initial data sequentially...');

    try {
        // First load - most critical (summary stats)
        await loadSummaryStats();

        // Second load - detection data
        await loadRecentDetections();

        // Third load - today's stats
...
async function loadRecentDetections() {
    try {
        const response = await fetch('/api/detections/recent?limit=50'); // Reduced from 100 to 50
        if (response.ok) {
...
// Optimized WebSocket connection with timeout
...
function startPollingFallback() {
    ...
    pollingInterval = setInterval(async () => {
        if (!isWsConnected && autoRefreshEnabled) {
            await refreshData();
        }
    }, 30000);
}

// Optimized summary stats update with throttling
    ...
    try {
        ...
        container.innerHTML = activities.map(item => {
            ...
            if (action.includes('login_success')) {
                icon = '✅';
                colorClass = 'success';
            } else if (action.includes('login_failed') || action.includes('unauthorized')) {
                icon = '❌';
                colorClass = 'error';
            ...
            return `
            <div class="user-activity-item ${colorClass}">
                ...
                <div class="user-info">
                    <div class="user-header">
                        <span class="user-name">${item.user || 'Anonymous'}</span>
                        <span class="activity-badge ${colorClass}">${item.action.replace(/_/g, ' ')}</span>
                    </div>
                    <div class="user-details">${item.details || ''}</div>
                </div>
                <div class="user-meta">
                    <div class="user-time">${formatTimeAgo(item.timestamp)}</div>
                    <div class="user-ip">${item.ip_address || ''}</div>
                </div>
            </div>
            `;
        }).join('');
...
function getUserAvatar(username) {
    const colors = ['#709138', '#00A1C9', '#2D3E50', '#fbbf24', '#ef4444'];
...
// Chart color palette
const chartColors = {
    primary: ['#709138', '#00A1C9', '#2D3E50', '#fbbf24', '#ef4444', '#709138', '#00A1C9', '#D1DBE5', '#4A4A4A', '#2D3E50'],
    gradient1: ['#709138', '#00A1C9'],
    violations: '#ef4444',
    safe: '#709138'
};

// Chart.js default configuration
Chart.defaults.color = '#2D3E50';
Chart.defaults.borderColor = '#D1DBE5';
...
function initializeCharts() {
    ...
    mainGradient.addColorStop(0, '#709138');
    mainGradient.addColorStop(1, '#00A1C9');
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    titleColor: '#f1f5f9',
                    bodyColor: '#cbd5e1',
                    borderColor: 'rgba(112, 145, 56, 0.3)',
                    borderWidth: 1,
                    cornerRadius: 8,
                    padding: 12
                }
        ...
        data: {
            ...
            datasets: [
                ...
                },
                {
                    ...
                    data: [],
                    borderColor: '#709138',
                    backgroundColor: 'rgba(112, 145, 56, 0.1)',
                    borderWidth: 3,
                    fill: true,
                    tension: 0.4,
                    pointRadius: 4,
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: chartColors.primary.map((color, index) =>
                    index < 4 ? `rgba(112, 145, 56, 0.8)` :
                        index < 8 ? `rgba(0, 161, 201, 0.8)` :
                            `rgba(45, 62, 80, 0.8)`
                ),
...
function changeMainChartType(chartType) {
    ...
    gradient.addColorStop(0, '#709138');
    gradient.addColorStop(1, '#00A1C9');
    ...
    charts.mainChart = new Chart(ctx, {
        ...
        options: {
            responsive: true,
            maintainAspectRatio: true,
            aspectRatio: newType === 'pie' || newType === 'doughnut' ? 1.5 : 2.5,
            plugins: {
                legend: {
                    position: 'bottom',
                    labels: {
