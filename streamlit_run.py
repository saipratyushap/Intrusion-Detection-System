...
def main_app():
    ...
    with col_detect_all:
        ...
        if st.button("All", key="btn_detect_all"):
            st.session_state.detect_all = True
        st.markdown('</div>', unsafe_allow_html=True)
    with col_detect_none:
        st.markdown('<div class="sidebar-button">', unsafe_allow_html=True)
        if st.button("None", key="btn_detect_none"):
    ...
    with col_alert_all:
        st.markdown('<div class="sidebar-button">', unsafe_allow_html=True)
        if st.button("All", key="btn_alert_all"):
            st.session_state.alert_all = True
        st.markdown('</div>', unsafe_allow_html=True)
    with col_alert_none:
        st.markdown('<div class="sidebar-button">', unsafe_allow_html=True)
        if st.button("None", key="btn_alert_none"):
    ...
    with tab1:
        ...
        with col_alert3:
            ...
            if st.button("🔔 Test Alert Sound", key="btn_test_alert_sound", use_container_width=True):
                try:
                    pygame.mixer.init()
                    pygame.mixer.music.load("alert.mp3")
                    pygame.mixer.music.play()
                    st.success("Alert sound playing!")
                except Exception as e:
                    st.error(f"Could not play sound: {e}")
            
            if st.button("📧 Test Email Alert", key="btn_test_email_alert", use_container_width=True):
                try:
                    from streamlit_run import send_violation_email_internal
                    result = send_violation_email_internal("test_object", 0.95, None)
                    if result:
                        st.success("Test email sent!")
                    else:
                        st.warning("Email not sent - check .env configuration")
                except Exception as e:
                    st.error(f"Error: {e}")
            
            st.markdown("</div></div>", unsafe_allow_html=True)
        
        # Show recent alerts from API
        if alert_count > 0:
            st.markdown("<br>", unsafe_allow_html=True)
            with st.expander("📋 Recent Alerts (Live from Backend)", expanded=True):
                try:
                    import requests as req_lib
                    response = req_lib.get("http://localhost:8000/api/alerts?limit=10", timeout=5)
                    if response.status_code == 200:
                        data = response.json()
    ...
    with tab5:
                    ...
                    with [col_rec1, col_rec2][j]:
                        with st.expander(f"🎬 {recording['name']}", expanded=False):
                            ...
                            try:
                                ...
                                # Try to play video with Streamlit
                                try:
                                    st.video(recording['path'])
                                except Exception as video_error:
                                    st.warning("Video playback not supported in browser. Please download to view.")
                                    st.info("💡 Tip: Download the video file and open it with a media player like VLC.")
        ...
        if recordings:
            ...
            with col_bulk1:
                if st.button("🗑️ Delete All Recordings"):
