...
# Authentication models (dummy for backward compatibility if needed)
...
async def get_home(request: Request):
    ...
    <head>
        ...
        <style>
            * {
            ...
            .logo {
                width: 100%;
                height: auto; /* Allow container to shrink to fit image */
                min-height: 80px;
                display: flex;
                align-items: center;
                justify-content: center;
                margin: 0 auto 30px; /* Increased bottom margin for spacing */
            ...
            .btn-primary {
                background: #709138;
                box-shadow: 0 10px 25px rgba(112, 145, 56, 0.2);
            }
            ...
            .btn-secondary {
                background: #00A1C9;
                box-shadow: 0 10px 25px rgba(0, 161, 201, 0.2);
            }
    ...
    <body>
        <div class="hero-container">
            <div class="logo">
                <img src="/static/logo.png?v=3" alt="ThirdEye Logo" style="max-height: 60px; width: auto; max-width: 100%;">
            </div>
            ...
            try:
                if os.path.exists(csv_file):
                    ...
                    if len(df) > last_line_count:
                        ...
                        # Fix: Check if there are any classes before getting most common
                    ...
                    last_line_count = 0 # Reset if file is deleted
            except Exception as e:
                print(f"Error in websocket loop: {e}")
                # Don't crash the whole endpoint on a single read error
...
async def get_snapshots():
    """List all detection snapshots with metadata."""
    snapshots_list = []
    try:
        if os.path.exists(frames_dir):
            ...
            # Limit to last 500 snapshots for performance
            for filename in files[:500]:
                filepath = os.path.join(frames_dir, filename)
                stats = os.stat(filepath)
                
                # Parse metadata from filename if possible
                # e.g., frame_20260211_095731_284253.jpg
                ...
                snapshots_list.append({
                    ...
                    "size": stats.st_size,
                    "class": "Unknown", # Would need to join with log for better info
                    "confidence": 0.0,
                    "violation": True # Assuming frames are mostly saved for violations
                })
...
async def delete_snapshot(request: Request):
    ...
    try:
        ...
        if not filename:
             return JSONResponse(status_code=400, content={"message": "Filename ID required"})
...
async def get_snapshots_gallery(request: Request):
    """Serve the snapshot gallery page"""
    return templates.get_template("snapshots.html").render(request=request)
...
def log_activity(user: str, action: str, details: str = "", ip_address: str = "", status: str = "success"):
    ...
    activity = {
        "id": str(len([]) + 1),  # This will be updated by the real function
        "timestamp": datetime.now().isoformat(),
        "user": user,
        "action": action,
        "details": details,
